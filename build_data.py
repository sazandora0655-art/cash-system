...
out.write_text(
    ...
    "window.EVE_DAYS = " + json.dumps(days, ensure_ascii=False, indent=1) + ";\n",
    encoding="utf-8")

# ── 残高スケジュール（人が確認する用） ──
L = ["# 残高スケジュール（AIトレード画面の正）", "",
...
for d in days:
    ...
    L.append("")
(HERE / "残高スケジュール.md").write_text("\n".join(L), encoding="utf-8")
print("days:", len(days), "→", out.name, "/ 残高スケジュール.md")
for d in days:
    print(f"  D{d['day']:>2} {d['date']} panes={d['panes']} n={len(d['trades']):>2} net={d['net']:>7,} close={d['balClose']:,}")
